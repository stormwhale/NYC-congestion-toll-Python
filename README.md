...
1) Use the EZPass registry to register vehicle plates in the database with Manhattan residency information.
   a) Plate number must be 3 letters followed by 4 digits
2) Use the EZPass scanner tool to scan vehicles with the EZPass transponder as vehicles passes the scanner.
   a) If no plate_db.csv file detect, the programm will generate a temporary file to store the plate information
   b) If plates scanned are not in the correct format, an error log will be created to track the information. A camera module is used to take pictures of the vehicle
   c) If plates scanned are not in the database, the vehicle is to be marked 'mail-in' to receive the mail-in toll price.
