...
def EZscanner_gui():
    ...
    def EZscanner():
        ...
        if len(plate) == 0:
            messagebox.showerror("Input required", "Please scan license plate.")
            plate_num.set("")
            return
    ...
    scan_bar_label = tk.Label(
        root, text="EZPass Scanner", font="Times 20", justify="center"
    )
    ...
    scan_bar_label.grid(row=0, column=0, padx=30, pady=10, sticky="w")
    scan_bar_entry.grid(row=1, column=0, padx=10, pady=10, sticky="n")
    scan_btn.grid(row=1, column=0, padx=70, pady=30, sticky="w")
    scanner_instruction_label.grid(row=1, column=10, pady=30)
